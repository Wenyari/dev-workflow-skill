...
```bash
node "{lark-tool-dir}/scripts/lark_publish_doc.mjs" \
  --title "客户管理页面开发技术方案" \
...
```bash
node "{lark-tool-dir}/scripts/lark_publish_doc.mjs" --title "标题" < document.md
```
...
```bash
node "{lark-tool-dir}/scripts/lark_check_permissions.mjs"
```
...
- 如果文档包含由当前环境真实可用且已授权的图形能力生成的飞书画板或图片链接，应在对应位置插入链接说明。
...
- **状态流转 / 流程**：用 ` ```mermaid ` 代码块（源码）。`lark-doc` 不渲染 Mermaid 成图；需要真图时，使用当前环境真实可用且已授权的图形能力生成产物后在文档中引用（见「图表规则」）。

## 图表规则

- `lark-doc` 不负责渲染 Mermaid。
- 需要图形化落地到飞书时，先使用当前环境真实可用且已授权的图形能力生成产物，再由 `lark-doc` 在文档中引用。
- 如果只有 Mermaid 源码，可以直接写入代码块。
- 当前环境没有可用的渲染能力时，保留 Mermaid 源码并明确标注“未渲染”；不得调用不存在的工具，也不得声称图已生成。
- 如果实际采用的飞书画板链路提示需要 `lark-cli`，再按该能力的要求初始化；不要为了普通文档读写提前初始化。
