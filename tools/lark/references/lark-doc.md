...
- 如果文档包含由当前环境真实、已授权的图形能力生成的飞书画板或图片链接，应在对应位置插入链接说明。
...
- **状态流转 / 流程**：用 ` ```mermaid ` 代码块（源码）。`lark-doc` 不渲染 Mermaid 成图；需要图形产物时按下方「图表规则」处理。

## 图表规则

- `lark-doc` 不负责渲染 Mermaid。
- 默认保留 Mermaid 源码并写入代码块，便于审核和后续修改。
- 只有用户明确要求图形化落地，且当前环境确实提供真实、已授权的飞书画板、文档或图片生成能力时，才调用该能力生成产物并由 `lark-doc` 引用。
- 当前环境没有可用图形能力时，保留源码并明确标注“未渲染”；不得调用未注册的工具，也不得声称已经生成图形产物。
- 图形能力自身明确要求额外准备时，再按该能力的真实说明执行；不要为了普通文档读写提前初始化无关工具。
