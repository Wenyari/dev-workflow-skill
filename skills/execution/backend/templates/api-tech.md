...
<!-- 先根据 mermaid.md 判断是否命中选图条件。正式文档只保留下面两种结构之一，并删除所有 HTML 注释。 -->

<!-- 选项 A：命中选图条件时，至少生成一张 Mermaid 主图。根据核心评审问题选择图型，不要机械生成所有图型。 -->
...
<!-- 选项 B：所有选图条件都未命中时，删除上面的图示目标、Mermaid 图和关键结论，只保留下面一行。 -->

**无需配图原因**：<本方案未命中选图条件，以及文字和表格已足以评审的具体原因>
# 数据模型 / 数据库设计
<!-- 【必写】 -->
...
<!-- 仅更新类接口保留下一项；查询、创建等非更新接口删除。不得只写“无”。 -->

**并发控制**：<使用 expectedUpdatedAt / expectedVersion / If-Match / CAS 条件更新；或说明不采用乐观锁的依据、风险和替代保护>
