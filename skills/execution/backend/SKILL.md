...
- 用户显式输入 `$backend api-tech`、`$backend integration-test generate [scope]` 或 `$backend integration-test run [scope]`
- 用户要求“写后端技术方案”“写接口和数据模型设计”
- 用户要求“给缺少测试的后端接口生成测试”或“运行指定接口、包或测试名称的集成测试”
- 用户要求“根据代码变更判断需要运行哪些后端接口测试”
