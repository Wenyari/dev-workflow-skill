---
name: frontend
description: 前端研发执行工作流。用户显式输入 `$frontend page-tech`、`$frontend page-build`、`$frontend page-build check`，或要生成页面技术方案、检查方案后创建页面基建、把已确认方案经过契约检查、人工确认、最小验证和基建 Review 落成可交接给视觉还原的前端代码时使用。
---
