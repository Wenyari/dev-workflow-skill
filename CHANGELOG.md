...
- 后端接口集成测试迁入 `backend` 的 `integration-test` 阶段，并改为根据仓库事实识别语言、框架、测试运行器和项目原生命令。
- 后端测试明确区分本地隔离环境、共享测试环境和生产或未知环境；共享环境写入、删除、消息及外部调用需要单独授权。
- `devFlow` 改为旧命令兼容导航。因尚未完成至少一个正式发布版本的兼容周期，本版本继续保留其 camelCase Skill ID 作为临时例外，不再向其中增加业务能力。
