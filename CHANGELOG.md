...
- 前端执行流程增加 `workload-plan` 命令、文档模板与结构检查脚本，用于在页面技术方案完成后生成工作量排期。
- Mermaid 时序图增加按业务域分组的模板与检查能力。

### Changed

- 更新前端页面技术方案的参考文档、模板和命令导航，使其与工作量排期流程衔接。
- 补充 Mermaid 检查器和后端接口技术方案检查脚本的测试覆盖。
